...
def plot_csv_wnominate(csv_file: str, output_file: Optional[str] = None, 
    ...
    for legislator_id in results["idpt"].keys():
        if legislator_id in metadata and metadata[legislator_id]['partido']:
            ...
            colors[legislator_id] = party_colors.get(party, party_colors["Default"])
        else:
    ...
    for legislator_id, coords in results["idpt"].items():
        ...
        plot_labels.append(labels.get(legislator_id, legislator_id) if show_labels else "")
    
    # Crear un diagrama de dispersión
    scatter = plt.scatter(x_vals, y_vals, c=plot_colors, s=60, alpha=0.7, edgecolors='black', linewidth=0.5)
    ...
    circle_x = 1.0 * np.cos(theta)  # R W-NOMINATE normalmente utiliza el círculo unitario
...
def compare_with_pynominate(csv_file: str, json_file: str, output_file: Optional[str] = None):
    ...
    if not os.path.exists(json_file):
        raise FileNotFoundError(f"Archivo JSON de pynominate no encontrado: {json_file}")
    ...
    for legislator_id, coords in r_results["idpt"].items():
        ...
        ax1.scatter(x, y, c=color, s=60, alpha=0.7, edgecolors='black', linewidth=0.5)
    ...
    if 'idpt' in py_results:
        for legislator_id, coords in py_results["idpt"].items():
            ...
            clean_id = legislator_id.replace('M', '') if legislator_id.startswith('M') else legislator_id
            party = metadata.get(clean_id, {}).get('partido', '')
            color = party_colors.get(party, party_colors["Default"])
            ax2.scatter(x, y, c=color, s=60, alpha=0.7, edgecolors='black', linewidth=0.5)
...
def main():
    ...
    try:
        if args.compare_json:
            # Crear gráfico de comparación
            compare_with_pynominate(args.csv_file, args.compare_json, args.output)
        else:
